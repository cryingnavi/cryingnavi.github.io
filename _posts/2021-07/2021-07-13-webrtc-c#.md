...
_webcamSource.I420AVideoFrameReady += LocalI420AFrameReady;
```
생성한 미디어 트랙은 피어 커넥션 객체의 AddTransceiver 메소드를 이용하여 지정될 것이다. Transceiver는 로컬과 원격 미디어를 연결하는 파이프로 피어간에 미디어를 전송하는 역할을 수행한다.

I420AVideoFrameReady 이벤트는 비디오 소스가 생성되면 호출되는 이벤트입니다. 
